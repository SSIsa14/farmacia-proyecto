    ...
    steps {
        ...
        script {
            withCredentials([string(credentialsId: 'sonarqube-backend-main', variable: 'SONAR_TOKEN')]) {
                // Espera la finalización de la tarea backend
                def status = ""
                timeout(time: 15, unit: 'MINUTES') {
                    while (status != "SUCCESS" && status != "FAILED") {
                        def resp = sh(script: "curl -s -u ${SONAR_TOKEN}: ${SONAR_HOST_URL}/api/ce/task?id=${backendTaskId}", returnStdout: true).trim()
                        def taskStatus = sh(script: "echo '${resp}' | jq -r '.task.status'", returnStdout: true).trim()
                        status = taskStatus
                ...
                // Verifica el Quality Gate del backend
                def qgStatus = sh(script: "curl -s -u ${SONAR_TOKEN}: ${SONAR_HOST_URL}/api/qualitygates/project_status?projectKey=FP:Backend_Prod | jq -r '.projectStatus.status'", returnStdout: true).trim()
                if (qgStatus != "OK") {
                    error "Backend Quality Gate failed: ${qgStatus}"
                } else {
                    echo "Backend Quality Gate OK"
                }
            }
        }
